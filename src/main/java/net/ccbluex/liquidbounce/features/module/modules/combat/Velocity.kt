...
class Velocity : Module() {
     ...
     * OPTIONS
     */
    private val horizontalValue = FloatValue("Horizontal", 0F, -1F, 1F, "x") {!modeValue.get().equals("Hypixel", true)}
    private val verticalValue = FloatValue("Vertical", 0F, -1F, 1F, "x") {!modeValue.get().equals("Hypixel", true)}
    private val horizontalExplosionValue = FloatValue("HorizontalExplosion", 0F, 0F, 1F, "x") {!modeValue.get().equals("Hypixel", true)}
    private val verticalExplosionValue = FloatValue("VerticalExplosion", 0F, 0F, 1F, "x") {!modeValue.get().equals("Hypixel", true)}
    private val modeValue = ListValue("Mode", arrayOf("Cancel", "Simple", "AACv4", "AAC4Reduce", "AAC5Reduce", "AAC5.2.0", "AAC", "AACPush", "AACZero",
            "Reverse", "SmoothReverse", "Jump", "Glitch", "Phase", "Matrix", "Legit",  "AEMine", "Jartex"), "Cancel") // later
    ...
    private val hypixel = BoolValue("Hypixel", false){ modeValue.get().equals("simple", true)}
    /**
     * VALUES
     */
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (packet is S12PacketEntityVelocity) {
            ...
            when (modeValue.get().toLowerCase()) {
                ...
                "jartex" -> event.cancelEvent()
                "simple" -> {
                    val horizontal = horizontalValue.get()
                    val vertical = verticalValue.get()
                    if(hypixel.get()){
                        if(mc.thePlayer.fallDistance > 0.10){
                            packet.motionX = (0).toInt()
                            packet.motionY = (0).toInt()
                            packet.motionZ = (0).toInt()
                        } else {
