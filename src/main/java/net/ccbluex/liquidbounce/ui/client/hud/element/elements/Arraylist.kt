...
class Arraylist(x: Double = -1.0, y: Double = 0.0, scale: Float = 1F,
                side: Side = Side(Horizontal.RIGHT, Vertical.UP)) : Element(x, y, scale, side) {
    private val colorModeValue = ListValue("Color", arrayOf("Custom", "Random", "Sky", "CRainbow", "LiquidSlowly", "Fade", "Mixer"), "CRainbow")
    private val blurValue = BoolValue("Blur", true)
    private val blurStrength = FloatValue("Blur-Strength", 30F, 0F, 30F, { blurValue.get() })
    ...
    val colorGreenValue = IntegerValue("Green", 153, 0, 255)
    ...
    private val cRainbowSecValue = IntegerValue("CRainbow-Seconds", 4, 1, 10)
